...
test.describe('Autenticação e Controle de Acesso', () => {
  ...
  test('CT05 - deve bloquear acesso à tela de admin se o colaborador estiver logado porém sem ser admin', async ({ page }) => {
    ...
    await page.locator('#senha')
      .fill('novaSenha123');

    await page.locator('#senha_confirmation')
      .fill('novaSenha123');

    await page.getByRole('button', { name: 'Salvar e Fazer Login' })
      .click();

    await expect(page.locator('body'))
      .toContainText('Senha atualizada com sucesso! Faça o login com sua nova senha.');
    ...
    await expect(page.locator('body'))
      .toContainText(/(403|Forbidden|Ação não autorizada|Unauthorized)/i);
  });
